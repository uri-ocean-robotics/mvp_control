...
namespace ctrl {
    ...
    // Number of constraints per thruster pair (ARTICULATED_THRUSTER_X + ARTICULATED_THRUSTER_Y)
    constexpr int NUM_CONSTRAINTS_PER_PAIR = 3;

    // Number of constraints per single thruster (FIXED_THRUSTER)
    constexpr int NUM_CONSTRAINTS_PER_SINGLE = 1;

    /** @brief Enumeration of possible thruster settings.
     *
     * This enum defines the different configurations that a thruster can have in the system.
     * - `FIXED_THRUSTER`: Non-articulated thruster that is fixed in position.
     * - `ARTICULATED_THRUSTER_X`: Articulated thruster decoupled along the X-axis.
     * - `ARTICULATED_THRUSTER_Y`: Articulated thruster decoupled along the Y-axis.
     */
    enum ThrusterSetting {
    FIXED_THRUSTER = 0,            // Non-articulated thruster
    ARTICULATED_THRUSTER_X = 1,    // Articulated thruster with X-axis alignment
     ...
     *
     * This enum defines the different types of constraints applied to each thruster
     * within the control allocation matrix. These constraints ensure that the thrusters
     * operate within their physical and operational limits.
     *
     * - `ROW_THRUST_FORCE_CONSTRAINT`: Ensures that the thrust force generated by the thruster
     *   remains within the specified lower and upper bounds.
     * - `ROW_ANGLE_CONSTRAINT_UPPER`: Limits the maximum allowable servo angle increase based on
     *   the servo's rotational speed and hardware angle limits.
     * - `ROW_ANGLE_CONSTRAINT_LOWER`: Limits the maximum allowable servo angle decrease based on
     *   the servo's rotational speed and hardware angle limits.
     */
    enum ConstraintRow {
        ROW_THRUST_FORCE_CONSTRAINT = 0,    // Constraint on thrust force
        ROW_ANGLE_CONSTRAINT_UPPER,         // Upper constraint on servo angle change
        ROW_ANGLE_CONSTRAINT_LOWER          // Lower constraint on servo angle change
    };
